...
class grid
{
  ...
  public:
    ...
    inline double get_dx()   { if(nx <= 1 ){return 0;}else{return (xmax - xmin)/(nx-1);} }
    inline double get_dy()   { if(ny <= 1 ){return 0;}else{return (ymax - ymin)/(ny-1);} }
    inline double get_dz()   { if(nz <= 1 ){return 0;}else{return (zmax - zmin)/(nz-1);} }
